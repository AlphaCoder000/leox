...
class McSeekerProfileView extends StatelessWidget {
        ...
        builder: (dialogContext, setState) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text("This action cannot be undone. Your user data will be permanently deleted.", style: TextStyle(fontSize: 14.sp)),
              ...
              ElevatedButton(
                ...
                onPressed: confirmationText.trim().toLowerCase() == 'delete'
                    ? () async {
                        ...
                        final seekerRef = FirebaseFirestore.instance.collection('mc_seekers').doc(context.read<McSeekerAuthController>().currentSeeker!.id);
                        await seekerRef.delete();
      ...
      body: seeker == null
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildProfileCard(theme, seeker),
                  SizedBox(height: 3.h),
  ...
  Widget _buildProfileCard(ThemeData theme, McSeekerModel seeker) {
    return Card(
        ...
        child: Column(
          children: [
            CircleAvatar(
              ...
              child: Text(
                seeker.userName.isNotEmpty ? seeker.userName.substring(0, 1).toUpperCase() : "?",
                style: TextStyle(fontSize: 26.sp, fontWeight: FontWeight.bold, color: const Color(0xFF0EA5E9)),
              ),
            ),
            SizedBox(height: 2.h),
            Text(seeker.userName, style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold)),
            SizedBox(height: 0.5.h),
            Text(seeker.email, style: TextStyle(fontSize: 14.sp, color: Colors.grey)),
            SizedBox(height: 2.h),
            const Divider(),
            SizedBox(height: 1.h),
            _infoRow(Icons.phone_outlined, seeker.phone),
            SizedBox(height: 1.h),
            _infoRow(Icons.location_on_outlined, seeker.address),
          ],
        ),
      ),
    );
